...
from scripts.utils import (
    get_input_path, load_config, load_table, make_output_dir,
    rename_to_canonical, save_tsv, setup_logging, stage_banner, split_cell,
)
import logging
import yaml
# ══════════════════════════════════════════════════════════════════════════════
...
def build_population_graph(
    ...
    #   hasFunctionalRole   : kept separate — domain=DomainType (not Occ),
    #                         32 values, primary query target
    #
    ...
    _op(hasFunctionalRole,   DomainType,        FuncCatCtx)
    ...
    _dp(funcTagsText,  DomainType,      XSD.string)
    ...
    for _, r in df.iterrows():
        ...
        # Optional: function_tags on occurrence level
        if occurrence_function_tags and _norm(r.get("function_tags")):
            for tag in _split(r.get("function_tags")):
                g.add((occ_u, hasFuncCat, _cv(FuncCatCtx, tag, "FuncCat_")))
